...
class new_cla extends process {
    ...
    function get_list2($start, $limit) {
        $new_list = $this->get_list("SELECT * FROM news_users ORDER BY news_id DESC LIMIT $start, $limit");
        return $new_list;
    }
...
if (isset($_POST['save'])) {
    ...
    if ($action == 'news_add') { /* --------------------------------------cai nay la phan them ban nhe-------- */
        ...
        } else if ($new_object->exist('news_users', 'news_title', $data['news_title'], 'news_id')) {
            //Kiem tra xem chung da ton tai trong CSDL chua
            $error['news_title'] = 'Title already exist!';
        }
        ...
        if (empty($error)) {
            $flag = $new_object->add('news_users', $data);
            if ($flag) {
